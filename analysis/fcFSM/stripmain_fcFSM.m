...
curveD=cell(nlengths,1);
shapesD=cell(nlengths,1);
curveG=cell(nlengths,1);
shapesG=cell(nlengths,1);
clas=cell(nlengths,1);
...
%    end
	...
	if sum(tempIdx_eigenalue)>neigs
		tempIdx_eigenalue(find(tempIdx_eigenalue,sum(tempIdx_eigenalue)-neigs,'last'))=false;
	end
    modeNum=sum(tempIdx_eigenalue);
	curveL{l}(1:modeNum,1)=lengths(l);
	curveL{l}(1:modeNum,2)=tempVec_eigenValue(tempIdx_eigenalue);
	shapesL{l}=C_L*tempMat_eigenMode(:,tempIdx_eigenalue);

	% D buckling
	[tempMat_eigenMode,tempMat_eigenValue]=eigs(C_D'*K*C_D,C_D'*Kg*C_D,size(C_D,2),'SM');
	tempVec_eigenValue=diag(tempMat_eigenValue);
	tempIdx_eigenalue=(tempVec_eigenValue>0);
	if sum(tempIdx_eigenalue)>neigs
		tempIdx_eigenalue(find(tempIdx_eigenalue,sum(tempIdx_eigenalue)-neigs,'last'))=false;
    ...
    modeNum=sum(tempIdx_eigenalue);
	curveD{l}(1:modeNum,1)=lengths(l);
	curveD{l}(1:modeNum,2)=tempVec_eigenValue(tempIdx_eigenalue);
	shapesD{l}=C_D*tempMat_eigenMode(:,tempIdx_eigenalue);
	
	% G buckling
	[tempMat_eigenMode,tempMat_eigenValue]=eigs(C_G'*K*C_G,C_G'*Kg*C_G,size(C_G,2),'SM');
	tempVec_eigenValue=diag(tempMat_eigenValue);
	tempIdx_eigenalue=(tempVec_eigenValue>0);
	if sum(tempIdx_eigenalue)>neigs
		tempIdx_eigenalue(find(tempIdx_eigenalue,sum(tempIdx_eigenalue)-neigs,'last'))=false;
	...
	dofL=size(C_L,2);
	dofD=size(C_D,2);
	phi=[C_L,C_D,C_G]\modes;
	L_sub=C_L*phi(1:dofL,:);
	D_sub=C_D*phi(dofL+1:dofL+dofD,:);
	G_sub=C_G*phi(dofL+dofD+1:end,:);
	Energy_All=sum((modes./2).*(K*mode),1);
	L_energyPtn=sum((L_sub./2).*(K*L_sub),1)./Energy_All.*100;
