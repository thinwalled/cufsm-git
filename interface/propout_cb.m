...
% case 8
%     % First yield shape parameters based on actual extreme fibers
% 
%     extfiber = extfibergeom(node,elem,xcg,zcg,thetap*pi/180);
% 
%     fyfig = figure;
%     name = ['CUFSM v',version,' -- First yield shape parameters'];
%     set(fyfig,'Name',name,'NumberTitle','off');
%     set(fyfig,'MenuBar','none');
%     set(fyfig,'Units','normalized');
%     set(fyfig,'Position',[0.20 0.25 0.55 0.35]);
% 
%     uicontrol(fyfig,...
%         'Style','text','units','normalized',...
%         'HorizontalAlignment','Center',...
%         'FontName','Arial','FontSize',13,...
%         'Position',[0.05 0.86 0.90 0.08],...
%         'String','First Yield Shape Parameters');
% 
%     row_y = [0.68 0.52 0.36 0.20];
%     row_label = {'x','z','1','2'};
% 
%     dval = [extfiber.dx extfiber.dz extfiber.d1 extfiber.d2];
% 
%     cpval = [extfiber.cx_p extfiber.cz_p extfiber.c1_p extfiber.c2_p];
%     cmval = [extfiber.cx_m extfiber.cz_m extfiber.c1_m extfiber.c2_m];
% 
%     bspval = [extfiber.beta_s.x_p extfiber.beta_s.z_p ...
%               extfiber.beta_s.one_p extfiber.beta_s.two_p];
% 
%     bsmval = [extfiber.beta_s.x_m extfiber.beta_s.z_m ...
%               extfiber.beta_s.one_m extfiber.beta_s.two_m];
% 
%     % for i=1:4
%     %     outstr = [...
%     %         'd' row_label{i} ' = ' num2str(dval(i)) ...
%     %         '     c' row_label{i} 'p = ' num2str(cpval(i)) ...
%     %         '     c' row_label{i} 'm = ' num2str(cmval(i)) ...
%     %         '     beta_s_' row_label{i} 'p = ' num2str(bspval(i)) ...
%     %         '     beta_s_' row_label{i} 'm = ' num2str(bsmval(i))];
%     % 
%     %     uicontrol(fyfig,...
%     %         'Style','text','units','normalized',...
%     %         'HorizontalAlignment','Left',...
%     %         'FontName','Courier','FontSize',11,...
%     %         'Position',[0.05 row_y(i) 0.90 0.08],...
%     %         'String',outstr);
%     % end
% 
%     row_y = [0.68 0.52 0.36 0.20];
%     row_label = {'x','z','1','2'};
% 
%     d_label   = {'d_x','d_z','d_1','d_2'};
%     cp_label  = {'c_{xp}','c_{zp}','c_{1p}','c_{2p}'};
%     cm_label  = {'c_{xm}','c_{zm}','c_{1m}','c_{2m}'};
%     bsp_label = {'\beta_{s,xp}','\beta_{s,zp}','\beta_{s,1p}','\beta_{s,2p}'};
%     bsm_label = {'\beta_{s,xm}','\beta_{s,zm}','\beta_{s,1m}','\beta_{s,2m}'};
% 
%     dval = [extfiber.dx extfiber.dz extfiber.d1 extfiber.d2];
% 
%     cpval = [extfiber.cx_p extfiber.cz_p extfiber.c1_p extfiber.c2_p];
...
%               extfiber.beta_s.one_m extfiber.beta_s.two_m];
% 
%     col_x = [0.05 0.22 0.39 0.58 0.78];
%     col_w = [0.14 0.14 0.14 0.17 0.17];
% 
%     for i=1:4
%         entries = {...
%             [d_label{i} ' = ' num2str(dval(i),5)], ...
%             [cp_label{i} ' = ' num2str(cpval(i),5)], ...
%             [cm_label{i} ' = ' num2str(cmval(i),5)], ...
%             [bsp_label{i} ' = ' num2str(bspval(i),5)], ...
%             [bsm_label{i} ' = ' num2str(bsmval(i),5)]};
% 
%         for j=1:5
%             uicontrol(fyfig,...
%                 'Style','text','units','normalized',...
...
%                 'FontName','Arial','FontSize',13,...
%                 'Interpreter','tex',...
%                 'Position',[col_x(j) row_y(i) col_w(j) 0.08],...
%                 'String',entries{j});
%         end
%     end
...
case 8
    ...
    set(fyfig,'MenuBar','none');
    set(fyfig,'Units','normalized');
    set(fyfig,'Position',[0.20 0.25 0.60 0.35]);

    ax = axes('Parent',fyfig,...
        'Units','normalized',...
        'Position',[0 0 1 1],...
        'Visible','off');

    text(ax,0.5,0.90,'First Yield Shape Parameters',...
        'HorizontalAlignment','center',...
    ...
    col_x = [0.03 0.23 0.38 0.53 0.69 0.86];
    
    for i=1:4
    
        text(ax,col_x(1),row_y(i),row_name{i},...
            'HorizontalAlignment','left',...
            'FontName','Arial',...
            'FontWeight','bold',...
            'FontSize',13);
        ...
        for j=1:5
            text(ax,col_x(j+1),row_y(i),entries{j},...
                'HorizontalAlignment','left',...
                'FontName','Arial',...
                'FontSize',13,...
                'Interpreter','tex');
