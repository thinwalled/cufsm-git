...
Fy=55;
b=8;
length_index_plotted=[12 19 30]
label=['Z-section with lips (AISI 2002 Ex. I-10)'];
%
%Figures
figure(1)
semilogx(curve(:,1),curve(:,2),'k.-')
axis([1 1000 0 0.5])
hold on,plot([b b],[0.12 0.18],'k:'),hold off
...
axesnum=axes('Units','normalized','Position',[0.21 0.6 0.25 0.25],'visible','off');
...
%distortional
axesshape=axes('Units','normalized','Position',[0.41 0.13 0.25 0.25],'visible','off');
scale=1;
lengthindex=length_index_plotted(2);
dispshap(undefv,node,elem,shapes(:,lengthindex,modeindex),axesshape,scale,springs);
title(['Distortional P_{cr}/P_y=',num2str(curve(lengthindex,2),'%2.2f')])
%LTB
axesshape=axes('Units','normalized','Position',[0.64 0.35 0.25 0.25],'visible','off');
scale=1;
