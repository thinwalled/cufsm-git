...
Fy=55;
b=8;
length_index_plotted=[8 20 30]
label=['Z-section modified'];
%
%Figures
figure(1)
semilogx(curve(:,1),curve(:,2),'k.-')
axis([1 1000 0 2.5])
hold on,plot([b b],[1.6 2.4],'k:'),hold off
...
springs=0;
%local
axesshape=axes('Units','normalized','Position',[0.14 0.29 0.25 0.25],'visible','off');
scale=-1;
...
%distortional
axesshape=axes('Units','normalized','Position',[0.39 0.12 0.25 0.25],'visible','off');
scale=1;
lengthindex=length_index_plotted(2);
dispshap(undefv,node,elem,shapes(:,lengthindex,modeindex),axesshape,scale,springs);
title(['Distortional M_{cr}/M_y=',num2str(curve(lengthindex,2),'%2.2f')])
%LTB
axesshape=axes('Units','normalized','Position',[0.66 0.18 0.25 0.25],'visible','off');
scale=1;
