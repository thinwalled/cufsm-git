...
axis([1 1000 0 4])
hold on,plot([b b],[2.0 3.0],'k:'),hold off
xlabel('half-wavelength (in.)')
ylabel('LF_{cr} (applied P=P_u=2.8k, M=M_u=0.59k-in)') 
hold on
...
axesnum=axes('Units','normalized','Position',[0.52 0.6 0.25 0.25],'visible','off');
...
title(label)
text(1.5,3*b/4,['applied P=P_u=2.8k +']);
text(1.5,2*b/4,['        M=M_u=0.59k-in']);
text(1.5,b/4,['P_y=',num2str(Py,'%4.2f'),'kips']);
text(1.5,0,['M_y=',num2str(Mzz_y,'%4.2f'),'in.-kips']);
%
...
title(['Local/Distortional LF_{cr}=',num2str(curve(lengthindex,2),'%2.2f')])
%distortional
...
dispshap(undefv,node,elem,shapes(:,lengthindex,modeindex),axesshape,scale,springs);
%title('Flexural')
title(['Flexural LF_{cr}=',num2str(curve(lengthindex,2),'%2.2f')])
