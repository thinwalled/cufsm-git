...
%
%minimum inputs
load panel_mxtopfree
freecurve=curve;
load panel_mxtopfix
Fy=50;
b=3;
length_index_plotted=[9 32]
label=['Panel (AISI 2002 Ex. I-14)'];
%
%Figures
figure(1)
semilogx(curve(:,1),curve(:,2),'k.-')
hold on,semilogx(freecurve(:,1),freecurve(:,2),':'),hold off
axis([1 100 0 1.8])
hold on,plot([b b],[0.3 0.5],'k:'),hold off
...
axesnum=axes('Units','normalized','Position',[0.14 0.7 0.5 0.25],'visible','off');
...
springs=0;
%local
axesshape=axes('Units','normalized','Position',[0.14 0.05 0.5 0.25],'visible','off');
scale=-0.5;
...
%distortional
axesshape=axes('Units','normalized','Position',[0.4 0.42 0.5 0.25],'visible','off');
scale=0.5;
