function [] = templatepic(node, elem, axesnum, geom, h, b1, d1, q1, b2, d2, q2, r1, r2, r3, r4, t, cz, center)
    ...
    %   2015 - templatepic() was re-written so that it could be an overlay on cross-section, this is a bit weird, but essentially hold is on and dimensions are added on top of the usual cross-section plot.

    % node:     [node # | x | z | dofx | dofz | dofy | dofrot | stress]
    % elem:     [elem # | nodei | nodej | t | mat #]

    % If center is not 1 then outer dimensions and inner radii came in and these need to be corrected to all centerline for the use of this template:
    if center == 1
