function [] = crossect(node, elem, axesnum, springs, constraints, flags)
    ...
    % flags:    [node # | element # | mat # | stress # | stresspic | coord | constraints | springs | origin | propaxis]
    ...
    % Plot the origin point (and basic x- and z-axis), if wanted:
    if originflag == 1
    ...
    springscale = 0.05 * max(max(abs(node(:, 2:3)))); % Spring scale (of the largest dimension)
    springpic = [0 0
                 4.5 0
                 5 -4
                 6 4
            ...
            for i = 1:size(springs, 1)
                ...
                if springs == 0
                    ...
                    if springs(i, 3) == 0 % Spring to ground
                        ...
                        if kw ~= 0 % z-direction spring (up-down)
                            plot(xi + springpic(:, 2), zi + springpic(:, 1), 'r')
                        end

                        if kv ~= 0 % y-direction spring (in-out)
                            plot(xi, zi, 'rs')
                        end
    ...
    % Plot a picture of the stress distribution, if wanted (this plot overrides all other plotting options):
    % if stresspicflag == 1
    ...
    if propaxisflag == 1
        ...
        % Plot the global xx- and zz-axis:
